...
const companySmokeEmail = process.env.UI_COMPANY_EMAIL || 'j.whitfield@vantara.com.au'
  ...
  for (const plan of rolePlans) {
    ...
    page.on('requestfailed', (request) => {
      ...
      if (url.includes('/login') || url.includes('/dashboard') || url.includes('/analytics') || url.includes('/reports') || url.includes('/submissions') || url.includes('/company/')) {
        apiErrors.push(`requestfailed ${request.method()} ${url} -> ${errorText || 'unknown'}`)
      }
    })
    ...
    if (apiErrors.length) {
      failures.push(...apiErrors.slice(0, 20))
    }
    if (jsErrors.length) {
